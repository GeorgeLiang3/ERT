...
def showERTData(data, vals=None,line = 0,ndev =0, **kwargs):
...
def drawERTData(ax, data, vals=None,Start = None,End = None, **kwargs):
    """Plot ERT data as pseudosection matrix (position over separation).
...
def midconfERT(data, ind=None, rnum=1, circular=False):
    ...
    Start/End : int
        1 Line of 3D Measurement, starting and ending index
...
#    xe = np.hstack((pg.x(data.sensorPositions()), np.nan))  # not used anymore
    ...
    if len(ux) * 2 > data.sensorCount():  # 2D with topography case
        ...
        de = np.median(np.diff(ux)).round(1) # distance between points
